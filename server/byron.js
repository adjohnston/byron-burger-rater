    ...
    Meteor.startup(function () {
      ...
      HTTP.get('https://api.foursquare.com/v2/venues/search?intent=global&query=Byron&limit=50&categoryId=' + byronId + '&client_id=' + clientId + '&client_secret=' + clientSecret + '&v=20150612', function (err, res) {
        ...
        var byrons = res.data.response.venues.filter(function (venue) {
              return venue.categories[0].id === byronId;
            });

        byrons.forEach(function (byron) {
          var photo = HTTP.get('https://api.foursquare.com/v2/venues/' + byron.id + '/photos?limit=1&client_id=' + clientId + '&client_secret=' + clientSecret + '&v=20150612');
          
          Byrons.update({
            _id: byron.id
          }, {$set: {
            address: byron.location.formattedAddress,
            photo:   photo.data.response.photos.items[0].prefix + '150x150' + photo.data.response.photos.items[0].suffix,
            votes:   0,
            ips:     []
          });
        });
      ...
      Meteor.methods({
        castVote: function (byronId, vote) {
          ...
          Byrons.update(byronId, {$inc: {votes: vote}, $push: {ips: userIp}});
        }
      });
    });
